...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, _ = x.shape
    H = h0.shape[-1]
    h = torch.zeros((N, T, H)).double()
    for t in range(x.shape[1]):
        xt = x[:, t , :]
        if t == 0:
            next_h, cache_s = rnn_step_forward(xt, h0, Wx, Wh, b)
            cache.append(cache_s)
        else:
            # h[:, t, :] = next_h
            next_h, cache_s = rnn_step_forward(xt, next_h, Wx, Wh, b)   
            cache.append(cache_s)
...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[0][0].shape[-1]
    dx = torch.zeros((N, T, D)).double()
    dWx = torch.zeros((D, H)).double()
    dWh = torch.zeros((H, H)).double()
    db = torch.zeros((H, )).double()
    for t in range(dh.shape[1]-1, -1, -1):
        if t == dh.shape[1] - 1:
            dx_s, dprev_h, dWx_s, dWh_s, db_s = rnn_step_backward(dh[:, t, :], cache[t])
        else:
            # print(dh[:, t, :].shape)
            # print(dprev_h.shape)
            dx_s, dprev_h, dWx_s, dWh_s, db_s = rnn_step_backward(dh[:, t, :] + dprev_h, cache[t])
        dx[:, t, :] = dx_s
        dWx += dWx_s
        dWh += dWh_s
        db  += db_s
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions, return_pred = False):
        ...
        rnn_out = self.rnn_proj(rnn_out) # (N, T, W)

        loss = temporal_softmax_loss(rnn_out, captions_out, ignore_index=self._null)
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(x.shape[1]):
            xt = x[:, t , :]
            if t == 0:
                next_h, next_c = self.step_forward(xt, h0, next_c)
            else:
