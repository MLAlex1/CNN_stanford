...
def loss_function(x_hat, x, mu, logvar):
    ...
    ###############################################################################
    N = mu.shape[0]
    # we simply see how close are the original image and image reconstructed
    # from the decoder using z coming from encoder.
    reconstruction_loss = nn.functional.binary_cross_entropy(x_hat, x, reduction='sum')
    # kl divergence as derived in the notebook
    kl_divergence = (-0.5 * (1 + logvar - mu**2 - torch.exp(logvar)**2)).sum()
    # total loss
